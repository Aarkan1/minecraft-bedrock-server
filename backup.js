...
const remoteBackupKeepCount =
    backupConfig["num-backups-to-keep-for-type"].remote;

// AWS STUFF
let s3 = null;
const serverProps = config["server-properties"];
const bucketName = `Minecraft-bedrock-backup.${serverProps["level-name"]}`
    .toLowerCase()
    .replace(/( |_)/g, "-")
    .replace(/(-+)/g, "-");
...
const additionalDirectoriesToBackup = ["behavior_packs", "resource_packs"];
...
async function _createBackupFromFileToCopyLength(
    fileToCopyLength,
    backupStartTime,
    backupType
) {
    ...
    const outputArchiveStream = fs.createWriteStream(
        `${BACKUP_FOLDER_PATH}/${outputArchiveFileName}`
    );
    archive.pipe(outputArchiveStream);
    await Promise.all(
        Object.keys(fileToCopyLength).map(async (fileName) => {
            ...
            if (
                !(await fs.pathExists(
                    `${SERVER_WORLDS_FOLDER_PATH}/${fileName}`
                ))
            ) {
    ...
    await Promise.all(
        additionalFilesToBackup.map(async (fileName) => {
            const source = `${SERVER_WORLDS_FOLDER_PATH}/${WORLD_NAME}/${fileName}`;
            if (await fs.pathExists(source)) {
                archive.file(source, { name: `${WORLD_NAME}/${fileName}` });
            }
        })
    );

    await Promise.all(
        additionalDirectoriesToBackup.map(async (directory) => {
            const source = `${SERVER_WORLDS_FOLDER_PATH}/${WORLD_NAME}/${directory}`;
            if (await fs.pathExists(source)) {
                archive.directory(source, `${WORLD_NAME}/${directory}`);
            }
        })
    );
    ...
    console.log(
        `Finished creating local backup of server state at ${new Date(
...
async function createBackup(backupFileListString, backupStartTime, backupType) {
    ...
    await _createBackupFromFileToCopyLength(
        fileToCopyLength,
        backupStartTime,
        backupType
    );
...
async function createUnscheduledBackup(backupStartTime) {
    ...
    const filePaths = getFilePathsSync(SERVER_WORLDS_FOLDER_PATH).map(
        (filePath) => filePath.replace(`${SERVER_WORLDS_FOLDER_PATH}/`, "")
    );
    const fileToCopyLength = {};
...
async function restoreLatestLocalBackup() {
    ...
    const restorableArchives = allArchives.filter((fileName) =>
        fileName.includes(BACKUP_TYPES.SCHEDULED)
    );
    const numForceStopArchives = allArchives.length - restorableArchives.length;
    if (numForceStopArchives > 0) {
        console.log(
            `WARNING: Skipped ${numForceStopArchives} archive(s) of type 'ON_FORCED_STOP' that were generated when the server exited ungracefully`
        );
    }
...
async function purgeOldLocalBackups() {
    ...
    const allArchives = await fs.readdir(BACKUP_FOLDER_PATH);
    await Promise.all(
        Object.keys(BACKUP_TYPES).map(async (backupType) => {
            const maxBackupTypes = localBackupKeepCount[backupType];
            const backupsToPurge = getBackupsToPurge(
                allArchives.filter((fileName) => fileName.includes(backupType)),
                maxBackupTypes
            );
            backupsToPurge.forEach(async (backupToPurge) => {
                await fs.remove(`${BACKUP_FOLDER_PATH}/${backupToPurge}`);
                console.log(
                    `Removed ${backupToPurge} from local backups folder due to parameters defined in config["backup"]["num-backups-to-keep-for-type"]["local"]`
                );
            });
        })
    );
        ...
        Object.keys(BACKUP_TYPES).map(async (backupType) => {
            ...
            backupsToPurge.forEach(async (backupToPurge) => {
                await s3
                    ...
                    .promise();
                console.log(
                    `Removed ${backupToPurge} from AWS S3 due to parameters defined in config["backup"]["num-backups-to-keep-for-type"]["remote"]`
                );
            });
        })
    );
...
const pushBackupToRemote = util.promisify((archiveName, callback) => {
    const readStream = fs.createReadStream(
        `${BACKUP_FOLDER_PATH}/${archiveName}`
    );
    var params = {
        Bucket: bucketName,
        Key: archiveName,
...
function getRemoteBackupsToDownload(allRemoteBackups, allLocalBackups) {
    const lists = Object.keys(BACKUP_TYPES).map((backupType) => {
        const maxBackupTypes = remoteBackupKeepCount[backupType];
        const localBackups = allLocalBackups.filter((fileName) =>
            fileName.includes(backupType)
        );
        const remoteBackups = allRemoteBackups.filter((fileName) =>
            fileName.includes(backupType)
        );
        ...
        const backupsToDownload = slicedBackupsOfType.filter(
            (backup) => !localBackups.includes(backup)
        );
...
async function downloadRemoteBackups() {
    if (s3) {
        ...
        const bucketContents = await s3
            ...
            .promise();
        const allRemoteArchives = bucketContents.Contents.map(
            (entry) => entry.Key
        );
        const allLocalArchives = await fs.readdir(BACKUP_FOLDER_PATH);
        const remoteBackupsToDownload = getRemoteBackupsToDownload(
            allRemoteArchives,
            allLocalArchives
        );
        await Promise.all(
            remoteBackupsToDownload.map(async (backup) => {
        ...
        console.log(
            `Downloaded ${remoteBackupsToDownload.length} backup(s) from from AWS S3 bucket ${bucketName}`
        );
        await purgeOldLocalBackups();
    }
}
...
async function getBackupSizeList() {
    ...
    return allArchiveNames
        .filter((archive) => !/\.gitkeep/.test(archive))
        .map((archiveName) => {
...
async function doesLockFileExistOrS3Disabled() {
    ...
    const bucketContents = await s3
        ...
        .promise();
    return await bucketContents.Contents.map((entry) => entry.Key).includes(
        BUCKET_LOCK_FILE_NAME
    );
}
...
const createLockFileIfS3Enabled = util.promisify((callback) => {
    if (s3) {
        ...
        s3.upload(params, function (err, data) {
            if (!err) {
                console.log(
                    `Successfully created lock file ${BUCKET_LOCK_FILE_NAME} in AWS S3`
                );
                callback();
            } else {
                console.log(
                    `Error created lock file ${BUCKET_LOCK_FILE_NAME} in AWS S3:`
                );
                console.error(err);
            }
        });
...
async function deleteLockFileIfExists() {
    if (s3) {
        await s3
            ...
            .promise();
        console.log(
            `Successfully removed lock file from S3 bucket: ${BUCKET_LOCK_FILE_NAME}`
        );
    }
}
